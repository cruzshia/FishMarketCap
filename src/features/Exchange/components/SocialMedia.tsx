...
function SocialItem({ icon, url }: { icon: React.ReactNode; url: string }) {
  return (
...
function SocailMeida(props: ExchangeInfo) {
  ...
  return (
    <Grid container flexDirection='column' sx={{ mt: '4px', wordBreak: 'break-word' }}>
      ...
      {props.facebook_url && <SocialItem icon={<FacebookIcon />} url={props.facebook_url} />}
      {props.telegram_url && <SocialItem icon={<TelegramIcon />} url={props.telegram_url} />}
      {props.reddit_url && <SocialItem icon={<RedditIcon />} url={props.reddit_url} />}
      {props.slack_url && <SocialItem icon={<Slack />} url={props.slack_url} />}
